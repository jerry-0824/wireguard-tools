...
static int kernel_get_device(struct wgdevice **device, const char *iface)
{
	...
	*device = NULL;
	...
	wg_peer = buf + SIZEOF_WG_IOCTL_INTERFACE;
	for (ULONG i = 0; i < wg_iface->PeersCount; ++i) {
		...
		if (wg_peer->Flags & WG_IOCTL_PEER_HAS_ENDPOINT) {
			if (wg_peer->Endpoint.Sa.sa_family == AF_INET)
				peer->endpoint.addr4 = wg_peer->Endpoint.Sa4;
			else if (wg_peer->Endpoint.Sa.sa_family == AF_INET6)
				peer->endpoint.addr6 = wg_peer->Endpoint.Sa6;
		}
		...
		wg_aip = (void *)wg_peer + SIZEOF_WG_IOCTL_PEER;
		for (ULONG j = 0; j < wg_peer->AllowedIPsCount; ++j) {
			...
			if (wg_aip->AddressFamily == AF_INET) {
				memcpy(&aip->ip4, &wg_aip->Address4, sizeof(aip->ip4));
				aip->cidr = wg_aip->Cidr;
			} else if (wg_aip->AddressFamily == AF_INET6) {
				memcpy(&aip->ip6, &wg_aip->Address6, sizeof(aip->ip6));
				aip->cidr = wg_aip->Cidr;
			}
			++wg_aip;
		}
		wg_peer = (WG_IOCTL_PEER *)ALIGN_UP_BY((ULONG_PTR)wg_aip, __alignof(WG_IOCTL_PEER));
	}
	*device = dev;
	errno = 0;
out:
...
static int kernel_set_device(struct wgdevice *dev)
{
	...
	WG_IOCTL_PEER *wg_peer;
	WG_IOCTL_ALLOWED_IP *wg_aip;
	size_t buf_len = SIZEOF_WG_IOCTL_INTERFACE;
	...
	for_each_wgpeer(dev, peer) {
		buf_len += SIZEOF_WG_IOCTL_PEER;
		for_each_wgallowedip(peer, aip)
			buf_len += ALIGN_UP_BY(sizeof(WG_IOCTL_ALLOWED_IP), __alignof(WG_IOCTL_PEER));
	}
	...
	peer_count = 0;
	wg_peer = (void *)wg_iface + SIZEOF_WG_IOCTL_INTERFACE;
	for_each_wgpeer(dev, peer) {
		...
		if (peer->endpoint.addr.sa_family == AF_INET) {
			wg_peer->Endpoint.Sa4 = peer->endpoint.addr4;
			wg_peer->Flags |= WG_IOCTL_PEER_HAS_ENDPOINT;
		} else if (peer->endpoint.addr.sa_family == AF_INET6) {
			wg_peer->Endpoint.Sa6 = peer->endpoint.addr6;
			wg_peer->Flags |= WG_IOCTL_PEER_HAS_ENDPOINT;
		}
		...
		for_each_wgallowedip(peer, aip) {
			...
			if (aip->family == AF_INET)
				wg_aip->Address4 = aip->ip4;
			else if (aip->family == AF_INET6)
				wg_aip->Address6 = aip->ip6;
			else
				continue;
			++aip_count;
			++wg_aip;
